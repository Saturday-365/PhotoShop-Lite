...
void medianFilter(const char *name, const char *new_name) {
  ...
  uint8 *data;
  readBMPInfo(name, width, height, data_offset, data_size, data); //读取BMP信息并写入信息
...
void shrinkImage(const char *name, const char *new_name, double ratio) {
  ...
  uint32 new_width = static_cast<uint32>(width * ratio);   //设置新的图像宽度
  uint32 new_height = static_cast<uint32>(height * ratio);  //设置新的图像高度
  uint32 num_width = data_size / height;
  ...
  while (true) {
    ...
    case '\r': // 回车键确认选择
      ...
      case 2: {
        ...
        uint8 *data1;
        readBMPInfo(bmp_name1, width1, height1, data_offset1, data_size1, data1);
        cout << "请输入旋转角度（例如 45）: ";
        cin >> angle;
